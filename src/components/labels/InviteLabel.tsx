...
const InviteLabel = ({
  ...
  const { data: planData, isLoading, isError } = useInvitePlanInfo(plan_id);
  return (
        ...
        <div className="w-full h-full flex items-center justify-center flex-wrap content-center overflow-y-scroll overflow-x-hidden flex-col">
          ...
          <span className="flex items-center justify-center flex-col">
            ...
            <h1 className="text-blue-200 text-center text-lg font-bold pb-2 ">
              {isLoading
                ? 'Loading...'
                : planData?.plan_createdAt
                ? new Date(planData?.plan_createdAt).toLocaleString('en-ZA')
                : 'No Data Found'}
            </h1>
          </span>
        </div>
...
function useInvitePlanInfo(plan_id: string) {
  return useQuery(['inviteLabelPlan', plan_id], async () => {
    const { data, error } = await supabase
      .from('user_plans')
      .select('plan_createdAt')
      .eq('plan_id', plan_id);
    ...
    return data[0] as { plan_createdAt: string };
  });
}
