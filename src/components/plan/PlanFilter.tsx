...
function PlanFilter({
  ...
  const configureUserInviteForInfo = async (invite_id: string) => {
    ...
    const { data: userData, error: userError } = await supabase
      .from('user_info')
      .select(
        `user_id,user_email,user_sepushID,user_plan_Invites,user_weekLSTimes`
      )
  ...
  const filterClassnames = classNames(
    'absolute flex p-1 right-0 h-[90vh] w-1/2 bg-slate-700 rounded-sm z-4'
  );
        ...
        <div className="flex flex-col space-y-2 w-full h-2/5 justify-evenly px-2 ">
          ...
          <div className="text-base rounded-xl w-full h-fit bg-gradient-to-r p-[4px] from-[#6EE7B7] via-[#3B82F6] to-[#9333EA]">
            <input
              ...
              value={minPlanTime}
              className={filterInputTextClassNames}
              placeholder="Set Min Plan Amount In Minutes"
              onChange={(e: ChangeEvent<HTMLInputElement>) => {
                const val = e.target.value;
                if (!val) {
                  toast.warning('Please Enter a number');
                  return setminPlanTime(40);
                }
                if (+val < 1) {
                  toast.warning('Please Enter a number greater than 0');
                  return setminPlanTime(30);
                }
                setminPlanTime(+val);
              }}
            />
